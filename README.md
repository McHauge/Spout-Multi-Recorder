...
The icon is generated by `go run ./tools/icongen` (writes `internal/assets/icon.png`, embedded as the Fyne window/taskbar icon). The exe icon comes from `rsrc_windows_amd64.syso`, regenerated with:

```bash
go install github.com/tc-hib/go-winres@latest
go-winres simply --icon internal/assets/icon.png --arch amd64 --product-name "Spout Multi Recorder"
```
